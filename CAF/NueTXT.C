...
void NueTXT()
{  
  const std::string fnamenc = "defname:prod_decaf_R17-03-01-prod3reco.l_fd_genie_fluxswap_rhc_nova_v08_full_nue_or_numu_or_nus_contain_v1 with limit 6300";
  ...
  const Cut kNCSelection(
			 [](const caf::SRProxy* sr)
			 {
			   ...
			   if(sr->mc.nu[0].iscc != 1)  return false;
			   if(sr->mc.nu[0].pdg  != 12) return false;
			   return true;
			 });   
  ...
  MakeTextListFile(fnamenc, {kNCSelection && kNus17EnergyCut && kNus17EventQuality && kNus17HitCut}, {"Nue.txt"}, vars, &kStandardSpillCuts);
}
#endif
