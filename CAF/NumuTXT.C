...
{  
  ...
  const Cut kNCSelection(
			 [](const caf::SRProxy* sr)
			 {
			   ...
			   if(sr->mc.nu[0].pdg  != 14) return false;
			   return true;
			 });   
  ...
  MakeTextListFile(fnamenc, {kNCSelection && kNus17EnergyCut && kNus17EventQuality && kNus17HitCut}, {"Numu.txt"}, vars, &kStandardSpillCuts);
}
#endif
